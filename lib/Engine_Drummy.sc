...
Engine_Drummy : CroneEngine {
	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		(0..5).do({arg i; 
			SynthDef("player"++i,{ arg t_trig=0, amp=0.0,rate=1,pan=0,lpf=20000,resonance=2,hpf=10,sampleStart=0,sampleLength=3,t_gate=0;
				var snd,bufsnd;
				...
				Out.ar(0,snd*EnvGen.ar(Env([0,1, 1, 0], [0.01,sampleLength,0.1]),gate:t_gate))
			}).add;	
		});
